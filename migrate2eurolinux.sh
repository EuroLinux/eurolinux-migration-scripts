...
almalinux-repos bcache-tools btrfs-progs centos-backgrounds \
centos-gpg-keys centos-indexhtml centos-linux-release \
centos-linux-repos centos-logos centos-release \
centos-release-advanced-virtualization centos-release-ansible26 \
centos-release-ansible-27 centos-release-ansible-28 \
centos-release-ansible-29 centos-release-azure \
centos-release-ceph-jewel centos-release-ceph-luminous \
centos-release-ceph-nautilus centos-release-ceph-octopus \
centos-release-configmanagement centos-release-cr \
centos-release-dotnet centos-release-fdio centos-release-gluster40 \
centos-release-gluster41 centos-release-gluster5 \
centos-release-gluster6 centos-release-gluster7 \
centos-release-gluster8 centos-release-gluster-legacy \
centos-release-messaging centos-release-nfs-ganesha28 \
centos-release-nfs-ganesha30 centos-release-nfv-common \
centos-release-nfv-openvswitch centos-release-openshift-origin \
centos-release-openstack-queens centos-release-openstack-rocky \
centos-release-openstack-stein centos-release-openstack-train \
centos-release-openstack-ussuri centos-release-opstools \
centos-release-ovirt42 centos-release-ovirt43 \
centos-release-ovirt44 centos-release-paas-common \
centos-release-qemu-ev centos-release-qpid-proton \
centos-release-rabbitmq-38 centos-release-samba411 \
centos-release-samba412 centos-release-scl centos-release-scl-rh \
centos-release-storage-common centos-release-virt-common \
centos-release-xen centos-release-xen-410 centos-release-xen-412 \
centos-release-xen-46 centos-release-xen-48 \
centos-release-xen-common centos-repos desktop-backgrounds-basic \
insights-client kernel-uek kernel-uek-core kernel-uek-modules \
libreport-centos libreport-plugin-mantisbt \
libreport-plugin-rhtsupport libreport-rhel \
libreport-rhel-anaconda-bugzilla libreport-rhel-bugzilla \
libdtrace-ctf linux-firmware-core oracle-backgrounds \
oracle-epel-release-el8 oracle-indexhtml oraclelinux-release \
oraclelinux-release-el7 oraclelinux-release-el8 \
oraclelinux-release-el9 oracle-logos python3-dnf-plugin-ulninfo \
python3-syspurpose python-oauth redhat-backgrounds \
Red_Hat_Enterprise_Linux-Release_Notes-7-en-US redhat-indexhtml \
redhat-logos redhat-release redhat-release-eula \
redhat-release-server redhat-support-lib-python \
redhat-support-tool rhc thnlib rocky-backgrounds rocky-gpg-keys \
rocky-indexhtml rocky-logos rocky-obsolete-packages rocky-release \
rocky-repos sl-logos uname26 yum-conf-extras yum-conf-repos)
...
there's always the risk of something going wrong in the process
and users are always recommended to make a backup."
    echo "Do you want to continue? Type 'YES' if that's the case."
    read answer
    if [[ ! "$answer" =~ ^[Yy][Ee][Ss]$ ]]; then
      exit_message "Confirmation denied since an answer other than
'YES' was provided, exiting."
    fi
  fi
}

dep_check() {
  # Several utilities are needed for migrating. They may also
  # differ in names and versions between Enterprise Linux
  # releases. Check if one of them ($1) exists and exit if it
  # doesn't.
  if ! command -v "$1"; then
      exit_message "'${1}' command not found. Please install or
add it to your PATH and try again."
  fi
}

exit_message() {
  # Wrap a generic message about a script error with EuroLinux'
  # GitHub URL and exit.
  echo "$1"
...
final_failure() {
  # A generalized exit message that will appear in case of a
  # disastrous event.  Wrapped as a function since it will be used
  # several times along with `trap` on critical operations that
  # are not easily revertible.
  exit_message "An error occurred while attempting to switch this
system to EuroLinux and it may be in an unstable/unbootable state.
To avoid further issues, the script has terminated."
}

check_fips() {
  if [ "$(grep 'fips=1' /proc/cmdline)" ]; then
    exit_message "You appear to be running a system in FIPS mode,
which is not supported for migration. This case requires an
individual migration approach that can be received by contacting
our technical department. Please use the medium our sales
department provided after a subscription has been purchased."
  fi
}

check_secureboot(){
  if grep -oq 'Secure Boot: enabled' <(bootctl 2>&1) ; then
    exit_message "You appear to be running a system with Secure
Boot enabled, which is not yet supported for migration. Disable it
first, then run the script again."
  fi
}

generate_rpms_info() {
  # Generate an RPM database log and a list of RPMs installed on
  # your system at any point in time.
  set +euo pipefail
...
check_root() {
  # The script must be ran with superuser privileges any way
  # possible. You can refer to the way described in README.md -
  # just switch to the root account and run with `bash
...
check_distro() {
  # Determine the exact Enterprise Linux flavor installed now
  # before a migration took place. It has to be a one and only one
  # specific match against our supported distros list.  This
  # function will check an RPM - your /etc/redhat-release
  # provider. No deep scans such as comparing mentions of other
  # distros in package names, configuration files, etc. will be
  # checked - it may turn out in some specific scenarios that
  # distro X had packages branded as distro Y and Z installed too
  # - but if they are branded, they'll be removed as listed in the
  # bad_packages array.
  echo "Checking your distribution..."
  if ! old_release=$(rpm -q --whatprovides /etc/redhat-release); then
      exit_message "You appear to be running an unsupported
distribution."
  fi

  if [ "$(echo "${old_release}" | wc -l)" -ne 1 ]; then
      exit_message "Could not determine your distribution because
...
check_supported_releases() {
  # Our supported distros list mentioned earlier in check_distro()
  # comments.  In here this check is generalized and the
  # old_release variable may be overridden later on once a more
  # specific check is performed (this will be explained later once
  # this override is performed).
  case "${old_release}" in
    redhat-release*) ;;
  ...
  case "${old_release}" in
    redhat-release*)
      ...
      if [ "$(subscription-manager list --consumed | grep 'No consumed subscription pools were found' )"  ] ; then
        echo "No consumed subscription pools were found.
Proceeding with migration."
      else
        exit_message "The system is registered. Please backup all
the subscription-related keys, certificates, etc. if necessary and
remove the system from subscription management service with
'subscription-manager unregister', then run this script again."
      fi
      ;;
  esac
  if [ "$preserve" != "true" ]; then
    # Delete third-party repos' packages as well unless the
    # 'preserve' option has been specified.
    set +e
...
check_yum_lock() {
  # Don't attempt to modify packages if there's an ongoing
  # transaction.
...
check_systemwide_python() {
  # This script has an embedded Python code for several operations
  # that are expressed better that way rather than via a shell. It
  # will need a Python interpreter. This check ensures the proper
  # locations and version of the interpreter (the exact
  # invocations will be used later in the script).  Once the
  # embedded Python code is present, it's written for that exact
  # system-wide interpreter and integrated with other system-wide
  # components - no incompatibilities as long as the system has
  # the tools installed from its official repositories and no
  # unofficial tweaks have been made (e.g.  replacing a
  # system-wide Python 2 with 3).
...
find_repos_directory() {
  # Store your package manager's repositories directory for later
  # use.
...
find_enabled_repos() {
  # Store your package manager's enabled repositories for later
  # use.
...
grab_gpg_keys() {
  # Get EuroLinux public GPG keys; store them in a predefined
  # location before adding any repositories.
...
create_temp_el_repo() {
  # Before the installation of our package that provides .repo
  # files, we need an information on where to get that and other
  # EuroLinux packages from, that are mandatory for some of the
  # first steps before a full migration (e.g.  registering to
  # EuroMan). A temporary repository that provides these packages
  # is created here and removed later after a migration succeeds.
  # There's no need to worry about the repositories' names - even
  # if they change in future releases, the URLs will stay the
  # same.  It's possible to use your own repository and provide
  # your own .repo file as a parameter - in this case no extras
  # are created.
  if [ -n "$path_to_internal_repo_file" ]; then
...
register_to_euroman() {
  # EuroLinux earlier than 8 requires a valid EuroMan account. The
  # script needs to know your account's credentials to register
  # the instance it's being ran on and migrate it successfully.
  # Some additional EuroLinux packages will have to be installed
  # for that too - that's the most important case of using the
  # temporary switch-to-eurolinux.repo repository. No packages
  # from other vendors can accomplish this task.  It's possible to
  # use your own repository and provide your own .repo file as a
  # parameter - in this case the registration process is skipped.
  if [ -z "$path_to_internal_repo_file" ]; then
    ...
    case "$os_version" in
      ...
      *)
        ...
        echo "Determining el_org_id based on your registration
name & password..."
        el_org_id=$(python2 -c "
import xmlrpclib
...
check_el_repos_connectivity() {
  # Perform a `yum makecache` to check if we can successfully
  # connect to EuroLinux repositories. Just an additional check
  # for safety in case there's something going on with the
  # connection so we know in advance to fix it now rather than
  # later.  The commands mentioned appear to return 0 anyway on
  # Enterprise Linux 8 so we need an additional logic for checking
  # for 404s rather than relying on status codes. The same logic
  # has been applied for Enterprise Linux 7 for consistency.
  if [ ! -n "$path_to_internal_repo_file" ]; then
    echo "Checking EuroLinux repositories connectivity..."
      ...
      case "$os_version" in
        8*|9*)
          ...
          ;;
        7*)
          echo "(The connectivity check may take a long time on
Enterprise Linux 7.)"
...
disable_distro_repos() {
  ...
  # Remove all non-Eurolinux .repo files unless the 'preserve'
  # option has been provided. If it was, then here's a summary of
  # the function's logic: Different distros provide their
  # repositories in different ways. There may be some additional
  # .repo files that are covered by distro X but not by distro Y.
  # The files may be provided by different packages rather than a
  # <distro>-release RPM. This function will take care of all
  # these inconsistencies to disable all the distro-specific
  # repositories.  This is the case mentioned in
  # check_supported_releases() comments about overriding the
  # old_release variable because of different .repo files'
  # provider for certain distros.  The procedure may be simplified
  # but not replaced by using a '*.repo' glob since there may be
  # some third-party repositories that should not be disabled such
  # as EPEL - only take care of another Enterprise Linux
  # repositories.
  ...
  if [ "$preserve" != "true" ]; then
    ...
    # Most distros keep their /etc/yum.repos.d content in the
    # -release rpm. Some do not and here are the tweaks for their
    ...
    # ... this one should apply to any Enterprise Linux except
    # RHEL:
...
remove_centos_yum_branding() {
  # CentOS provides their branding in /etc/yum.conf. As of
  # 2021.09.03 no other distro appears to do the same but if this
  # changes, equivalent branding removals will be provided here.
...
fix_oracle_shenanigans() {
  # Several packages in Oracle Linux have a different naming
  # convention. These are incompatible with other Enterprise
  # Linuxes and treated as newer versions by package managers. For
  # a migration we need to 'downgrade' them to EuroLinux
  # equivalents once EuroLinux repositories have been added.
  #
  # Some Oracle Linux exclusive packages with no equivalents will
  # be removed as well. Oracle-branded enabled modules will be
  # reset here and their names stored for later use once a
  # distro-sync has been performed.
  set +euo pipefail
...
force_el_release() {
  # Get yumdownloader if applicable and force and installation of
  # el-release, removing the current release provider package.
  set +euo pipefail
  case "$os_version" in
      8*|9*)
  ...
  # Additional tweak for RHEL 8 - remove these directories
  # manually.  Otherwise an error will show up: error: unpacking
  # of archive failed on file [...]: cpio: File from package
  # already exists as a directory in system
  if [[ "$old_release" =~ redhat-release ]]; then
...
disable_el_repos_if_custom_repo_is_provided() {
  # If a custom repo file is provided, do not use the repos we
  # install with el-release. This also applies if one wants to use
  # our Vault and migrate from an Enterprise Linux 8.4 (old
  # version) to EuroLinux 8.4.
  if [ -n "$path_to_internal_repo_file" ]; then
    case "$os_version" in
      8.6|8.7|8.8|8.9|8.10|9*)
        # Disabling the repos for offline migration is applicable
        # to EL8 only since on EL7 the core repos are skipped
        # along with skipping EuroMan registration. These below
        # are provided by the el-release package.
        echo "Disabling the EuroLinux 8/9-provided repos for offline
migration..."
        dnf config-manager --disable {baseos,appstream,powertools}
        ;;
      8.3|8.4|8.5)
        echo "Disabling the EuroLinux 8-provided 'certify' repos
for offline migration..."
        dnf config-manager --disable certify-{baseos,appstream,powertools}
        ;;
      7*)
        echo "EuroLinux 7 core repos have been already skipped
along with skipping EuroMan registration by using a custom .repo"
        ;;
    esac
  fi
}
...
install_el_base() {
  # Remove packages from other Enterprise Linux distros and
  # install ours. It's vital that this be performed in one go such
  # as with `yum shell` so the important dependencies are replaced
  # with ours rather than failing to be removed by a package
  # manager.
...
EOF
      then
        exit_message "Could not install base packages. Run 'yum
distro-sync' to manually install them."
      fi
      ;;
    7*)
      yum reinstall -y \*
...
el_distro_sync() {
  # Make sure all packages are synchronized with the ones
  # EuroLinux provides.
...
restore_modules() {
  ...
  # First, check if the system the migration is taking place on
  # was Oracle Linux - otherwise it's pointless to run all these
  # checks.
  if [[ "$old_release" =~ oracle ]]; then
    ...
    case "$os_version" in
      8*)
        if [ -n "${oracle_modules_enabled[*]}" ]; then
            ...
            echo "Reenabling the modules:
${eurolinux_modules_to_enable[*]}..."
        ...
        if [ -n "${oracle_modules_installed[*]}" ]; then
            ...
            echo "Reinstalling the modules:
${eurolinux_modules_to_install[*]}..."
...
deal_with_problematic_rpms() {
  # Some RPMs are either not covered by 'replaces' metadata or
  # couldn't be replaced earlier. This part takes care of all of
  # them.  In some cases these can be replaced automatically but
  # until additional tests have been performed, this logic is kept
  # here.
  ...
  # libzstd - required during the migration process, can be
  # removed now
  yum remove -y libzstd || true

  # A necessary downgrade to the version from our repos since the
  # 'virt' module gets installed when debranding Oracle Linux
  # modules. It may fail and a removal is performed as a fallback
  # action.
  case "$os_version" in
    8*)
...
fix_reinstalled_rpms() {
  # Reinstalling OpenJDK packages breaks their links in
  # /etc/alternatives:
  # https://bugzilla.redhat.com/show_bug.cgi?id=1976053
  rpm -qa --scripts java-*-openjdk-* | sed -n '/postinstall/, /exit/{ /postinstall/! { /exit/ ! p} }' | sh

  # After reinstalling IPA we need to perform an 'upgrade' to make
  # it work again.
  set +euo pipefail
...
compare_all_rpms() {
  ...
  # Once an internal .repo file is provided, search for the names
  # of the offline repositories and construct them as a grep
  # pattern. Take a look at the pipe symbol: | before a command
  # substitution takes place - it will be used for appending the
  # result to `grep -Ev [...]` and will work even if the variable
  # is nonexistent.
  if [ -n "$path_to_internal_repo_file" ]; then
    internal_repo_pattern="|$(grep -oP '\[\K[^\]]+' "$path_to_internal_repo_file" | xargs echo | sed 's/ /|/g')"
  fi

  # Query all packages and their metadata such as their Vendor.
  # The result of the query will be stored in a Bash array named
  # non_eurolinux_rpms[...].  Since earlier EuroLinux packages are
  # branded as Scientific Linux, an additional pattern is
  # considered when looking up EuroLinux products.  Some packages
  # may not be branded properly - we use `yum` to determine their
  # origin and then check with `rpm`.  When listing packages with
  # `yum`, there may be a few which are listed with two lines
  # rather than one due to their long filename - the output is
  # modified via `sed` to deal with this curiosity.  To complicate
  # things even further, two packages (rhnlib and rhnsd) are not
  # branded. They are excluded from the non-EuroLinux RPM list.
  ...
  if [[ -n "${non_eurolinux_rpms_and_metadata[*]}" ]]; then
    echo "The following non-EuroLinux RPMs are installed on the
system:"
    printf '\t%s\n' "${non_eurolinux_rpms_and_metadata[@]}"
    echo "This may be expected of your environment and does not
necessarily indicate a problem. If a large number of RPMs from
other vendors are included and you're unsure why please open an
issue on ${github_url}"
    if [ "$preserve" != "true" ]; then
      echo "Removing these packages (except those kernel-related)
automatically..."
...
remove_distro_gpg_pubkey() {
  ...
  if [ "$preserve" == "true" ]; then
    # We need to make sure only the pubkeys of the vendors that
    # provide the distros we're migrating from are removed and
    # only these. As of today the solution is to have an array
    # with their emails and make sure the corresponding pubkeys
    # are removed.
    bad_providers=('packager@almalinux.org' 'security@centos.org' \
'build@oss.oracle.com' 'security@redhat.com' \
'infrastructure@rockylinux.org' 'scientific-linux-devel@fnal.gov')
    for provider in ${bad_providers[*]} ; do
      echo "Checking for the existence of gpg-pubkey provider:
$provider..."
      grep -i $provider <<< "$keys" | cut -d' ' -f 1 | xargs rpm -e || true
    done
  else
    # On the other hand if we want to remove everything not
    # related to EuroLinux, remove all of these keys unless they
    # end with '@euro-linux.com'
    echo "Checking for the existence of non-EuroLinux gpg-pubkeys
and removing them..."
...
update_bootloader() {
  ...
  echo "Updating the GRUB2 bootloader at $grub2_conf (symlinked to
$(readlink $grub2_conf))."
...
remove_leftovers() {
  # Remove all temporary files and tweaks used during the
  # migration process.
...
remove_kernels_and_related_packages() {
  # The answer on what to remove See the remove_kernels.sh's
...
congratulations() {
  echo "Switch almost complete. EuroLinux recommends rebooting
this system.  Once booted up, a background service will perform a
further kernel removal."
}
