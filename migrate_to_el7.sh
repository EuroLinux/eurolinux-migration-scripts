...
#
# description: Migrates existing {RHEL,OL,CentOS,Scientific Linux} 7 installation to EuroLinux 7
...
# Using the prefix 'MY_' in order to avoid any variable naming collisions.
...
dump_rpms(){
    # This is a case when we need to manually revert changes, also useful for debugging
    echo "DUMPING installed RPMs' names:" | tee -a $MY_LOGFILE
...
set_repos(){
    ...
    #
    echo "You haven't replaced the 'TODO-FIXME' placeholders and the "
    echo "precautionary exit call, which the README informs about. Edit "
    echo "this script according to README instructions."
    exit 1
...
change_release(){
    ...
    yum info el-release
    echo "Checking the package that provides system-release"
    sys_release=$(rpm -q --whatprovides redhat-release)
    if [ -z "$sys_release" ]; then
        echo "Unable to find system-release. Is your distribution supported?"
        exit 1
    fi
    ...
    case "$sys_release" in
        redhat-release*|centos-release*|sl-release*|oraclelinux-release*|enterprise-release*)
            echo "Found supported distro: $sys_release!" ;;
